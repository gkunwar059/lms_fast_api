...
class User(Base):
    ...
    def get_current_user(token: str = Depends(User_JwtBearer())):
        """decode the token and fetch user details(email) from the database"""
        user_email = Auth.decode_user_token(token)
        if not user_email:
            raise HTTPException(
        ...
        if not user:  # else return the exception
            raise HTTPException(
        ...
        return {
            "email": user.email,
            "role": user.role_id,
        }  # dictonaries keys are : email and role remmber it
